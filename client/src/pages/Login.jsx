...
const handleLogin = async (e) => {
  ...
  try {
    const response = await axios.post(`${server}/api/v1/login`, { email, password }, {
  ...
  const handleMFASubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await axios.post(`${server}/api/v1/verify-mfa`, { otp }, { withCredentials: true });
      if (response.data.success) {
  ...
  return (
    <>
      ...
      <div className="flex justify-center items-center">
        <div className="w-full max-w-sm p-4 bg-gray-800 rounded-lg shadow sm:p-6 md:p-8 mt-28">
          <form className="space-y-6" onSubmit={handleLogin}>
            <h5 className="text-2xl font-medium text-white">Sign in to <span className='text-green-500'>SecureKey&#60;&#47;&#62;</span></h5>
            <div>
        ...
        {/* MFA Modal - Only show during login process, not during initial auth check */}
        {isMFAModalOpen && (
